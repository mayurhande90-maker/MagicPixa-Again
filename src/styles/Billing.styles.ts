...
export const BillingStyles = {
  container: "p-[min(3vh,24px)] sm:p-[min(4vh,32px)] lg:p-[min(6vh,48px)] pb-32 w-full max-w-7xl mx-auto animate-fadeIn",
  ...
  // Credit Card (Hero)
  creditCard: "bg-gradient-to-br from-[#4D7CFF] to-indigo-600 text-white p-[min(5vh,40px)] rounded-[2.5rem] shadow-xl relative overflow-hidden mb-[min(4vh,48px)]",
  ...
  creditBigNumber: "text-[clamp(48px,10vh,84px)] font-black leading-none",
  
  // Packs Grid
  packGrid: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[min(3vh,24px)] items-stretch mt-8",
  packCard: `bg-white p-[min(3.5vh,32px)] rounded-[2rem] shadow-sm border-2 text-left flex flex-col transition-all duration-300 transform hover:-translate-y-2 relative`,
  ...
  // Buttons
  packButton: `w-full mt-auto py-[min(1.8vh,14px)] rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2`,
  ...
  historyItem: "flex justify-between items-center text-sm p-[min(1.5vh,14px)] rounded-xl hover:bg-gray-50 transition-colors group border border-transparent hover:border-gray-100",
