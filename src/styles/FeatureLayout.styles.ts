...
export const FeatureStyles = {
  ...
  wrapper: "flex flex-col h-full p-4 lg:p-6 bg-[#FFFFFF] overflow-hidden",
  
  // Header
  header: "flex-none mb-4 border-b border-gray-100 pb-3",
  titleRow: "flex items-center gap-4 mb-1",
  iconContainer: "p-2.5 bg-gray-50 rounded-xl border border-gray-100 shadow-sm",
  titleText: `text-xl lg:text-2xl font-bold ${Theme.colors.textPrimary}`,
  description: "text-xs lg:text-sm text-gray-500 font-medium max-w-3xl",
  
  // Grid Container - Takes remaining space
  gridContainer: "flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8",
  ...
  controlsBox: "bg-[#F6F7FA] p-5 rounded-3xl flex flex-col h-full border border-gray-100 overflow-hidden",
  controlsHeader: "flex-none flex items-center justify-between mb-4",
  controlsTitle: "text-xs font-bold text-gray-400 uppercase tracking-wider",
  controlsScrollArea: "flex-1 overflow-y-auto custom-scrollbar pr-1 relative",
  
  // Generate Button Area
  actionArea: "flex-none pt-5 border-t border-gray-200 bg-[#F6F7FA] z-10",
  generateButton: `group w-full text-base lg:text-lg font-bold py-3.5 lg:py-4 rounded-2xl shadow-lg transition-all transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none flex items-center justify-center gap-3 active:scale-95 ${Theme.colors.primary} ${Theme.colors.textOnPrimary} ${Theme.colors.primaryHover} shadow-yellow-500/20`,
  ...
  // Input Components
  inputLabel: "block text-[10px] lg:text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1",
  inputField: "w-full px-4 py-3 lg:px-5 lg:py-4 bg-white border-2 border-gray-100 hover:border-gray-300 focus:border-[#4D7CFF] rounded-2xl outline-none transition-all font-medium text-sm lg:text-base text-[#1A1A1E] placeholder-gray-400",
  
  // Selection Grid
  selectionContainer: "mb-5 animate-fadeIn",
  selectionButton: "px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl text-[10px] lg:text-xs font-bold border transition-all duration-300 transform active:scale-95",
