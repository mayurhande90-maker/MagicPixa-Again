...
export const MagicRealty: React.FC<{ auth: AuthProps; appConfig: AppConfig | null; navigateTo: (page: Page, view?: View) => void }> = ({ auth, appConfig, navigateTo }) => {
    ...
    const handleNewSession = () => { setModelImage(null); setPropertyImage(null); setReferenceImage(null); if (brandStrategy === 'custom') setLogoImage(null); setModelChoice(null); setPropertyChoice(null); setTexts({ projectName: '', unitType: '', marketingContext: '', location: '', price: '', rera: '', contact: '' }); setAmenities([]); setShowAmenities(false); setShowContact(false); setResultImage(null); setDetectedFields(null); setLastCreationId(null); };
            ...
            <FeatureLayout 
                ...
                leftContent={
                    <div className="relative h-full w-full flex items-center justify-center p-4 bg-white rounded-3xl border border-dashed border-gray-200 overflow-hidden group mx-auto shadow-sm">
                        {loading ? (<div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn"><div className="w-64 h-1.5 bg-gray-700 rounded-full overflow-hidden shadow-inner mb-4"><div className="h-full bg-gradient-to-r from-indigo-400 to-blue-500 animate-[progress_2s_ease-in-out_infinite] rounded-full"></div></div><p className="text-sm font-bold text-white tracking-widest uppercase animate-pulse">{designMode === 'auto' ? "Pixa is researching trends..." : "Pixa is cloning layout..."}</p></div>) : (<div className="text-center opacity-50 select-none"><div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4"><BuildingIcon className="w-10 h-10 text-indigo-500" /></div><h3 className="text-xl font-bold text-gray-300">Realty Canvas</h3><p className="text-sm text-gray-300 mt-1">Your flyer will appear here.</p></div>)}<style>{`@keyframes progress { 0% { width: 0%; margin-left: 0; } 50% { width: 100%; margin-left: 0; } 100% { width: 0%; margin-left: 100%; } }`}</style></div>
                }
                rightContent={
                    isLowCredits ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 animate-fadeIn bg-red-50/50 rounded-2xl border border-red-100"><h3 className="text-xl font-bold text-gray-800 mb-2">Insufficient Credits</h3><button onClick={() => navigateTo('dashboard', 'billing')} className="bg-[#F9D230] text-[#1A1A1E] px-8 py-3 rounded-xl font-bold hover:bg-[#dfbc2b]">Recharge Now</button></div>) : (
                        <div className="space-y-8 p-1 animate-fadeIn">
                            {auth.user?.brandKit && (<div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-100"><div className="flex items-center gap-2 mb-3"><BrandKitIcon className="w-4 h-4 text-purple-600"/><label className="text-xs font-bold text-purple-800 uppercase tracking-wider">Brand Strategy</label></div><div className="flex gap-2"><button onClick={() => setBrandStrategy('brand_kit')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${brandStrategy === 'brand_kit' ? 'bg-white border-purple-400 text-purple-700 shadow-sm' : 'bg-transparent border-transparent text-gray-500 hover:bg-white/50'}`}>Use Brand Kit</button><button onClick={() => setBrandStrategy('custom')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${brandStrategy === 'custom' ? 'bg-white border-purple-400 text-purple-700 shadow-sm' : 'bg-transparent border-transparent text-gray-500 hover:bg-white/50'}`}>Custom / One-off</button></div></div>)}
                            <div><div className="flex items-center gap-2 mb-3 ml-1"><span className="bg-indigo-100 text-indigo-700 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">1</span><label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Lifestyle Model</label></div><div className="grid grid-cols-3 gap-2 mb-3"><StepCard title="Upload" description="Own photo" icon={<UserIcon className="w-4 h-4"/>} selected={modelChoice === 'upload'} onClick={() => setModelChoice('upload')} /><StepCard title="Generate" description="AI Human" icon={<SparklesIcon className="w-4 h-4"/>} selected={modelChoice === 'generate'} onClick={() => setModelChoice('generate')} /><StepCard title="Skip" description="Architecture only" icon={<XIcon className="w-4 h-4"/>} selected={modelChoice === 'skip'} onClick={() => { setModelChoice('skip'); setModelImage(null); }} /></div>{modelChoice === 'upload' && <CompactUpload label="Model Photo" image={modelImage} onUpload={handleUpload(setModelImage)} onClear={() => setModelImage(null)} icon={<UploadTrayIcon className="w-6 h-6 text-blue-400"/>} />}{modelChoice === 'generate' && (<div className="animate-fadeIn space-y-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100"><SelectionGrid label="Composition" options={compositionTypes} value={modelComposition} onChange={setModelComposition} />{modelComposition && <SelectionGrid label="Model Type" options={modelTypes} value={modelType} onChange={setModelType} />}{modelType && <SelectionGrid label="Region" options={modelRegions} value={modelRegion} onChange={setModelRegion} />}{modelRegion && <SelectionGrid label="Skin Tone" options={skinTones} value={skinTone} onChange={setSkinTone} />}{skinTone && <SelectionGrid label="Body Type" options={bodyTypes} value={bodyType} onChange={setBodyType} />}{bodyType && <SelectionGrid label="Shot Type" options={shotTypes} value={modelFraming} onChange={setModelFraming} />}</div>)}</div>
                            <div className="border-t border-gray-100 pt-6"><div className="flex items-center gap-2 mb-3 ml-1"><span className="bg-indigo-100 text-indigo-700 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">2</span><label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Property Asset</label></div><div className="grid grid-cols-2 gap-3 mb-3"><StepCard title="Upload Photo" description="Enhance existing" icon={<UploadTrayIcon className="w-5 h-5"/>} selected={propertyChoice === 'upload'} onClick={() => setPropertyChoice('upload')} /><StepCard title="Generate New" description="Create from text" icon={<SparklesIcon className="w-5 h-5"/>} selected={propertyChoice === 'generate'} onClick={() => { setPropertyChoice('generate'); setPropertyImage(null); }} /></div>{propertyChoice === 'upload' && <CompactUpload label="Site/Interior Photo" image={propertyImage} onUpload={handleUpload(setPropertyImage)} onClear={() => setPropertyImage(null)} icon={<BuildingIcon className="w-6 h-6 text-indigo-400"/>} />}</div>
                            <div className="border-t border-gray-100 pt-6"><div className="flex items-center gap-2 mb-3 ml-1"><span className="bg-indigo-100 text-indigo-700 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">3</span><label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Design Strategy</label></div><div className="grid grid-cols-2 gap-3 mb-3"><StepCard title="Reference" description="Clone a layout" icon={<LightbulbIcon className="w-5 h-5"/>} selected={designMode === 'reference'} onClick={() => setDesignMode('reference')} /><StepCard title="Pixa Auto-Design" description="Best practices 2025" icon={<MagicWandIcon className="w-5 h-5"/>} selected={designMode === 'auto'} onClick={() => { setDesignMode('auto'); setReferenceImage(null); }} /></div>{designMode === 'reference' && (<div className="animate-fadeIn relative">{analyzingRef && (<div className="absolute top-0 right-0 flex items-center gap-2 bg-blue-50 px-2 py-1 rounded-lg border border-blue-100 z-10"><div className="w-2 h-2 bg-blue-500 rounded-full animate-ping"></div><span className="text-[10px] text-blue-600 font-bold animate-pulse">Scanning Layout...</span></div>)}<CompactUpload label="Reference Layout Image" image={referenceImage} onUpload={handleRefUpload} onClear={() => { setReferenceImage(null); setDetectedFields(null); }} icon={<LightbulbIcon className="w-6 h-6 text-yellow-500"/>} heightClass="h-40" /></div>)}{designMode === 'auto' && (<div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-xs text-indigo-800 leading-relaxed"><p><strong>Pixa Agent Active:</strong> We will research current luxury real estate design trends and automatically generate a high-conversion layout for you.</p></div>)}</div>
                            <div className="border-t border-gray-100 pt-6"><div className="flex items-center gap-2 mb-3 ml-1"><span className="bg-indigo-100 text-indigo-700 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">4</span><label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Ad Details</label></div><div className="mb-4">{brandStrategy === 'brand_kit' && logoImage ? (<div className="flex items-center gap-3 p-3 bg-purple-50 rounded-xl border border-purple-100"><img src={logoImage.url} className="w-10 h-10 object-contain" alt="Brand Logo"/><div><p className="text-xs font-bold text-purple-900">Brand Kit Active</p><p className="text-[10px] text-purple-600">Logo & Contact pre-filled</p></div><button onClick={() => setBrandStrategy('custom')} className="ml-auto text-[10px] text-purple-500 underline hover:text-purple-700">Override</button></div>) : (<CompactUpload label="Brand Logo" image={logoImage} onUpload={handleUpload(setLogoImage)} onClear={() => setLogoImage(null)} icon={<SparklesIcon className="w-6 h-6 text-purple-400"/>} optional={true} heightClass="h-24" />)}</div><div className="grid grid-cols-1 gap-4"><div className="grid grid-cols-2 gap-3"><InputField label={<LabelWithBadge label="Project Name" detected={detectedFields?.hasProjectName} />} placeholder="e.g. Skyline Towers" value={texts.projectName} onChange={(e: any) => setTexts({...texts, projectName: e.target.value})} /><InputField label={<LabelWithBadge label="Unit Size" detected={detectedFields?.hasUnitType} />} placeholder="e.g. 2 BHK" value={texts.unitType} onChange={(e: any) => setTexts({...texts, unitType: e.target.value})} /></div><TextAreaField label="Context (Pixa will create title)" placeholder="e.g. 'Sea facing, Luxury'" value={texts.marketingContext} onChange={(e: any) => setTexts({...texts, marketingContext: e.target.value})} rows={3} /><InputField label={<LabelWithBadge label="Price (Optional)" detected={detectedFields?.hasPrice} />} placeholder="e.g. ₹1.5 Cr+" value={texts.price} onChange={(e: any) => setTexts({...texts, price: e.target.value})} /></div></div>
                            <div className="border-t border-gray-100 pt-4"><button onClick={handleToggleAmenities} className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${showAmenities ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'} border`}><div className="flex items-center gap-2"><CheckIcon className={`w-4 h-4 ${showAmenities ? 'text-blue-600' : 'text-gray-400'}`}/><span className="text-xs font-bold uppercase tracking-wider">Add Amenities?</span></div>{showAmenities ? <ChevronUpIcon className="w-4 h-4"/> : <ChevronDownIcon className="w-4 h-4"/>}</button>{showAmenities && (<div className={RealtyStyles.amenitiesContainer}>{amenities.map((amenity, idx) => (<div key={idx} className={RealtyStyles.amenityRow}><input type="text" placeholder={`Amenity ${idx + 1}`} value={amenity} onChange={(e) => updateAmenity(idx, e.target.value)} className={RealtyStyles.amenityInput}/><button onClick={() => removeAmenity(idx)} className={RealtyStyles.amenityRemove}><TrashIcon className="w-4 h-4"/></button></div>))}<button onClick={addAmenity} className={RealtyStyles.addAmenityBtn}><PlusCircleIcon className="w-4 h-4"/> Add Amenity</button></div>)}</div>
                            <div className="border-t border-gray-100 pt-4"><button onClick={() => { setShowContact(!showContact); autoScroll(); }} className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${showContact ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'} border`}><div className="flex items-center gap-2"><CheckIcon className={`w-4 h-4 ${showContact ? 'text-blue-600' : 'text-gray-400'}`}/><span className="text-xs font-bold uppercase tracking-wider">Add Footer Info?</span></div>{showContact ? <ChevronUpIcon className="w-4 h-4"/> : <ChevronDownIcon className="w-4 h-4"/>}</button>{showContact && (<div className="mt-4 grid grid-cols-1 gap-3 animate-fadeIn bg-gray-50 p-4 rounded-xl border border-gray-100"><InputField label="Location" placeholder="Address" value={texts.location} onChange={(e: any) => setTexts({...texts, location: e.target.value})} /><InputField label="Contact / Website" placeholder="Website" value={texts.contact} onChange={(e: any) => setTexts({...texts, contact: e.target.value})} /><InputField label="RERA ID" placeholder="Legal ID" value={texts.rera} onChange={(e: any) => setTexts({...texts, rera: e.target.value})} /></div>)}</div>
                        </div>
                    )
                }
            />
