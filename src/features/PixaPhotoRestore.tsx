...
export const PixaPhotoRestore: React.FC<{ auth: AuthProps; appConfig: AppConfig | null; navigateTo: (page: Page, view?: View) => void }> = ({ auth, appConfig, navigateTo }) => {
    ...
    const handleGenerate = async () => {
        ...
        try { const res = await colourizeImage(image.base64.base64, image.base64.mimeType, restoreMode, auth.activeBrandKit, auth.user?.basePlan || undefined); const blobUrl = await base64ToBlobUrl(res, 'image/png'); setResultImage(blobUrl); try { const dataUri = `data:image/png;base64,${res}`; const creationId = await saveCreation(auth.user.uid, dataUri, 'Pixa Photo Restore'); setLastCreationId(creationId); const updatedUser = await deductCredits(auth.user.uid, cost, 'Pixa Photo Restore'); if (updatedUser.lifetimeGenerations) { const bonus = checkMilestone(updatedUser.lifetimeGenerations); if (bonus !== false) setMilestoneBonus(bonus); } auth.setUser(prev => prev ? { ...prev, ...updatedUser } : null); } catch (persistenceError) { console.warn("Persistence/Credit deduction failed:", persistenceError); } } catch (e) { console.error(e); alert("Restoration failed. Please try again with a simpler image."); } finally { setLoading(false); }
    };
    ...
    const handleRefine = async (refineText: string) => {
        ...
        try {
            ...
            const res = await refineStudioImage(currentB64.base64, currentB64.mimeType, refineText, "Restored Historical Photo", auth.user?.basePlan || undefined);
