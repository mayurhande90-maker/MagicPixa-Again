...
import { PixaHeadshotIcon, UploadIcon, XIcon, UserIcon, CheckIcon, BuildingIcon, SparklesIcon, HomeIcon, PaletteIcon, PlusIcon, CreditCoinIcon, ArrowRightIcon, UsersIcon } from '../components/icons';
...
export const MagicHeadshot: React.FC<{ auth: AuthProps; appConfig: AppConfig | null; navigateTo: (page: Page, view?: View) => void }> = ({ auth, appConfig, navigateTo }) => {
    ...
    const handleGenerate = async () => {
        // Validation
        if (mode === 'single' && !personA) return;
        if (mode === 'duo' && (!personA || !personB)) return;
        if (!auth.user) return;
        ...
        try {
            ...
            const config = {
                mode,
                persona: archetypeData.label,
                location: backgroundData.label
            };

            const res = await generateProfessionalHeadshot(
                personA!.base64,
                mode === 'duo' && personB ? personB.base64 : null,
                config
            );
    ...
    const canGenerate = (mode === 'single' ? !!personA : (!!personA && !!personB)) && !isLowCredits;
                    ...
                    <div className="relative h-full w-full flex flex-col items-center justify-center p-4 bg-white rounded-3xl border border-dashed border-gray-200 overflow-hidden group mx-auto shadow-sm">
                        ...
                        {!personA && !personB ? (
                            <div className="text-center opacity-50 select-none"><div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4"><PixaHeadshotIcon className="w-10 h-10 text-indigo-500" /></div><h3 className="text-xl font-bold text-gray-300">Headshot Canvas</h3><p className="text-sm text-gray-300 mt-1">Upload a photo to begin.</p></div>
                             ...
                             // Visual representation of uploaded assets
                             <div className="relative w-full h-full flex items-center justify-center">
                                {personA && (
                                    <div className={`${HeadshotStyles.visualCard} ${mode === 'duo' && personB ? '-translate-x-6 -rotate-3' : ''}`} style={{ zIndex: 1 }}>
                                        <img src={personA.url} className="w-full h-full object-cover" />
                                        <div className={HeadshotStyles.visualLabel}>{mode === 'duo' ? 'Person A' : 'Subject'}</div>
                                    </div>
                                )}
                                {mode === 'duo' && personB && (
                                    <div className={`${HeadshotStyles.visualCard} translate-x-6 rotate-3`} style={{ zIndex: 2 }}>
                                        <img src={personB.url} className="w-full h-full object-cover" />
                                        <div className={HeadshotStyles.visualLabel}>Person B</div>
                                    </div>
                                )}
                             </div>
                        )}
                    ...
                    isLowCredits ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 animate-fadeIn bg-red-50/50 rounded-2xl border border-red-100"><CreditCoinIcon className="w-16 h-16 text-red-400 mb-4" /><h3 className="text-xl font-bold text-gray-800 mb-2">Insufficient Credits</h3><p className="text-gray-500 mb-6 max-w-xs text-sm">Requires {cost} credits.</p><button onClick={() => navigateTo('dashboard', 'billing')} className="bg-[#F9D230] text-[#1A1A1E] px-8 py-3 rounded-xl font-bold hover:bg-[#dfbc2b] transition-all shadow-lg">Recharge Now</button></div>) : (
                        <div className="space-y-6 p-2 animate-fadeIn">
                            ...
                            <div className="flex bg-gray-100 p-1 rounded-xl">
                                <button onClick={() => setMode('single')} className={`flex-1 py-2.5 rounded-lg text-xs font-bold uppercase transition-all ${mode === 'single' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                                    <UserIcon className="w-4 h-4 inline-block mr-2 mb-0.5" /> Individual
                                </button>
                                <button onClick={() => setMode('duo')} className={`flex-1 py-2.5 rounded-lg text-xs font-bold uppercase transition-all ${mode === 'duo' ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                                    <UsersIcon className="w-4 h-4 inline-block mr-2 mb-0.5" /> Partners / Duo
                                </button>
                            </div>
                            ...
                            <PremiumCard className="relative overflow-visible">
                                <div className="flex justify-between items-center mb-5">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em]">Source Photos</h3>
                                    </div>
                                </div>
                                <div className={`grid ${mode === 'duo' ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}>
                                    <PremiumUpload label={mode === 'duo' ? "Person A" : "Your Photo"} uploadText="Upload Photo" image={personA} onUpload={handleUpload(setPersonA)} onClear={() => setPersonA(null)} icon={<UserIcon className="w-6 h-6 text-indigo-300"/>} />
                                    {mode === 'duo' && (
                                        <PremiumUpload label="Person B" uploadText="Upload Photo" image={personB} onUpload={handleUpload(setPersonB)} onClear={() => setPersonB(null)} icon={<UserIcon className="w-6 h-6 text-pink-300"/>} />
                                    )}
                                </div>
                            </PremiumCard>
