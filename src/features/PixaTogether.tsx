...
const RefundModal: React.FC<{ onClose: () => void; onConfirm: (reason: string) => void; isProcessing: boolean; }> = ({ onClose, onConfirm, isProcessing }) => {
    const [reason, setReason] = useState(''); const reasons = ["Faces are distorted / blurry", "Didn't preserve identity well", "Glitch or Artifacts in image", "Unrealistic lighting/blending", "Other"];
    return (<div className="fixed inset-0 z-[250] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn" onClick={onClose}><div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl relative m-4" onClick={e => e.stopPropagation()}><button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><XIcon className="w-5 h-5"/></button><h3 className="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2"><FlagIcon className="w-5 h-5 text-red-500"/> Report Quality Issue</h3><p className="text-sm text-gray-500 mb-4">We're sorry the magic didn't work perfectly. Select a reason to request a refund.</p><div className="space-y-2 mb-6">{reasons.map(r => (<button key={r} onClick={() => setReason(r)} className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold border transition-all ${reason === r ? 'bg-red-50 border-red-500 text-red-700' : 'bg-gray-50 border-gray-100 text-gray-600 hover:bg-gray-100'}`}>{r}</button>))}</div><div className="flex gap-3"><button onClick={onClose} className="flex-1 py-3 text-gray-500 font-bold text-xs hover:bg-gray-50 rounded-xl transition-colors">Cancel</button><button onClick={() => onConfirm(reason)} disabled={!reason || isProcessing} className="flex-1 py-3 bg-[#1A1A1E] text-white rounded-xl font-bold text-xs hover:bg-black transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">{isProcessing ? <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"/> : "Submit & Refund"}</button></div></div></div>);
};
...
export const PixaTogether: React.FC<{ auth: AuthProps; appConfig: AppConfig | null; navigateTo: (page: Page, view?: View) => void }> = ({ auth, appConfig, navigateTo }) => {
    ...
    return (
        <>
            <FeatureLayout 
                title="Pixa Together" description="Reality Simulator. Merge two people into any scene, era, or art style." icon={<PixaTogetherIcon className="w-14 h-14"/>} rawIcon={true} creditCost={cost} isGenerating={loading} canGenerate={canGenerate} onGenerate={handleGenerate} resultImage={resultImage} onResetResult={handleGenerate}  onNewSession={handleNewSession} resultHeightClass="h-[1000px]" hideGenerateButton={isLowCredits}
                ...
                rightContent={
                    isLowCredits ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 animate-fadeIn bg-red-50/50 rounded-2xl border border-red-100"><CreditCoinIcon className="w-16 h-16 text-red-400 mb-4" /><h3 className="text-xl font-bold text-gray-800 mb-2">Insufficient Credits</h3><button onClick={() => navigateTo('dashboard', 'billing')} className="bg-[#F9D230] text-[#1A1A1E] px-8 py-3 rounded-xl font-bold hover:bg-[#dfbc2b] transition-all shadow-lg">Recharge Now</button></div>) : (
                        <div className="space-y-8 p-1 animate-fadeIn">
                            <div><label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 block ml-1">1. Select Engine</label><div className="grid grid-cols-3 gap-3"><ModeCard title="Creative" description="Full Control" icon={<SparklesIcon className="w-5 h-5"/>} selected={mode === 'creative'} onClick={() => setMode('creative')} /><ModeCard title="Reenact" description="Copy Pose" icon={<CameraIcon className="w-5 h-5"/>} selected={mode === 'reenact'} onClick={() => setMode('reenact')} /><ModeCard title="Pro Duo" description="Corporate" icon={<ShieldCheckIcon className="w-5 h-5"/>} selected={mode === 'professional'} onClick={() => setMode('professional')} /></div></div>
                            <div><div className="flex items-center justify-between mb-3 ml-1"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">2. Upload Subjects</label></div><div className="grid grid-cols-2 gap-4 mb-4"><CompactUpload label="Person A" image={personA} onUpload={handleUpload(setPersonA)} onClear={() => setPersonA(null)} icon={<UserIcon className="w-6 h-6 text-blue-400"/>} /><CompactUpload label="Person B" image={personB} onUpload={handleUpload(setPersonB)} onClear={() => setPersonB(null)} icon={<UserIcon className="w-6 h-6 text-pink-400"/>} /></div>{mode === 'reenact' && (<div className="animate-fadeIn mt-6"><div className="flex items-center gap-2 mb-3 ml-1"><div className="w-1.5 h-1.5 bg-purple-500 rounded-full animate-pulse"></div><label className="text-[10px] font-bold text-purple-600 uppercase tracking-widest">Reference Pose Required</label></div><CompactUpload label="Target Pose / Scene" image={referencePose} onUpload={handleUpload(setReferencePose)} onClear={() => setReferencePose(null)} icon={<CameraIcon className="w-6 h-6 text-purple-400"/>} heightClass="h-32" /><p className="text-[10px] text-gray-400 mt-2 px-1">Upload a photo with the exact pose or composition you want to copy.</p></div>)}</div>
                            {mode === 'creative' && (<div className="space-y-6 animate-fadeIn"><div className="h-px bg-gradient-to-r from-transparent via-gray-200 to-transparent"></div><SelectionGrid label="3. Relationship" options={relationshipOptions} value={relationship} onChange={(val) => { setRelationship(val); autoScroll(); }} /><SelectionGrid label="4. Environment" options={envOptions} value={environmentPreset} onChange={(val) => { setEnvironmentPreset(val); autoScroll(); }} />{environmentPreset === 'Custom' && (<div className="mt-2 animate-fadeIn"><InputField placeholder="Describe place (e.g. Hogwarts Great Hall)" value={customEnvironment} onChange={(e: any) => setCustomEnvironment(e.target.value)} /></div>)}<SelectionGrid label="5. Pose Guide" options={poseOptions} value={pose} onChange={setPose} /><SelectionGrid label="6. Clothing" options={clothingOptions} value={clothingMode} onChange={(val: any) => setClothingMode(val)} /><TextAreaField label="7. Custom Instructions (Optional)" placeholder="E.g. 'Make it golden hour', 'Person A is holding a rose', 'Cinematic cyberpunk vibe'" value={customDescription} onChange={(e: any) => setCustomDescription(e.target.value)} rows={3} /></div>)}
                            {mode === 'professional' && (<div className={PixaTogetherStyles.proModeBanner}><p className="font-bold mb-2">Professional Mode Active</p><p className="text-blue-700/80">Pixa will automatically:</p><ul className="list-disc list-inside mt-2 space-y-1 text-blue-700"><li>Dress subjects in premium business attire.</li><li>Generate a modern office or studio background.</li><li>Apply soft, flattering studio lighting.</li><li>Ensure confident, professional poses.</li></ul></div>)}
                        </div>
                    )
                }
            />
            ...
            {showRefundModal && <RefundModal onClose={() => setShowRefundModal(false)} onConfirm={handleRefundRequest} isProcessing={isRefunding} />}
