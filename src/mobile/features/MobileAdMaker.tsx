...
export const MobileAdMaker: React.FC<{ auth: AuthProps; appConfig: AppConfig | null; onGenerationStart: () => void; setActiveTab: (tab: View) => void }> = ({ auth, appConfig, onGenerationStart, setActiveTab }) => {
    ...
    const isStepAccessible = (idx: number): boolean => {
        ...
        if (idx === currentStep + 1) {
            ...
            // Config is optional, so it's always "filled" for progression purposes
            if (currentStepData.id === 'config') return true;
                                    ...
                                    {AD_STEPS.map((step, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span className={`font-bold uppercase tracking-tighter transition-all truncate w-full text-center px-0.5 min-h-[12px] flex items-center justify-center ${isActive || isFilled ? 'text-indigo-600' : 'text-gray-300'} ${isFlashing ? 'animate-pulse text-purple-600 text-[9px]' : 'text-[6px]'}`}>
