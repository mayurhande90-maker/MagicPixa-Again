...
const BrandAwarenessPill: React.FC<{ brand: BrandKit }> = ({ brand }) => (
    <div className="inline-flex items-center gap-3 px-6 py-2.5 bg-green-50/30 backdrop-blur-md border border-green-200 rounded-full shadow-sm animate-fadeIn transform transition-all hover:scale-[1.02] hover:bg-green-50 cursor-default group/pill">
        <div className="relative shrink-0">
            <div className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>
            <div className="absolute inset-0 bg-green-400 rounded-full blur-md opacity-20 group-hover/pill:opacity-40 transition-opacity"></div>
        </div>
        <span className="text-[12px] font-medium text-black whitespace-nowrap tracking-tight">
            Following <span className="font-bold text-black border-b-2 border-green-200/60">{brand.companyName || brand.name}</span> brand guidelines
        </span>
    </div>
);
