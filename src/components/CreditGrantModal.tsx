...
export const CreditGrantModal: React.FC<CreditGrantModalProps> = ({ userId, amount, message }) => {
    const [isClaimed, setIsClaimed] = useState(false);

    const handleClaim = async () => {
        setIsClaimed(true);
        // Wait for visual confirmation before removing from DB
        setTimeout(async () => {
             ...
             <div className="relative bg-gradient-to-br from-indigo-600 to-purple-700 w-full max-w-sm p-8 rounded-3xl shadow-2xl text-center transform animate-bounce-slight text-white border border-white/10" onClick={e => e.stopPropagation()}>
                 {!isClaimed ? (
                     <div className="animate-fadeIn">
                         ...
                         <h2 className="text-2xl font-bold mt-4 mb-2">Credits Granted!</h2>
                         <p className="text-indigo-100 mb-6 text-sm leading-relaxed px-2">
                             {message || "You've received a special bonus from the MagicPixa team."}
                         </p>
                     ...
                     <div className="animate-[fadeInUp_0.5s_ease-out] py-4">
                         ...
                         <h2 className="text-3xl font-bold mb-2 text-white">Added!</h2>
                         <p className="text-indigo-200 text-sm mb-6">Your balance has been updated.</p>
                     </div>
                 )}
             </div>
        </div>,
