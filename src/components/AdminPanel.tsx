...
import { AuthProps, AppConfig, User, Purchase, AuditLog, Announcement, ApiErrorLog, CreditPack, Ticket } from '../types';
import { 
    getAllUsers, 
    addCreditsToUser, 
    updateAppConfig, 
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ auth, appConfig, onConfigUpdate }) => {
    ...
    const loadUsers = async () => { setIsLoading(true); try { const users = await getAllUsers(); setAllUsers(users); } catch (e: any) { console.error("Failed to load users:", e); alert(`Error loading users: ${e.message}`); } finally { setIsLoading(false); } };
    ...
    const loadFeedback = async () => {
        setIsLoading(true);
        try {
            const feed = await getRecentFeedbacks(100); // Fetch more to enable meaningful stats
            setFeedbacks(feed);
        } catch (e) { console.error(e); }
        setIsLoading(false);
    };
    ...
    // ... (Existing Functions for tickets, announcements, config, etc.) ...
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex bg-white rounded-lg p-1 border border-gray-200">
                                    <button 
                                        ...
                                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${feedbackFilter === 'all' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                                    >
                                        All
                                    </button>
                                    <button 
                                        onClick={() => setFeedbackFilter('up')} 
                                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${feedbackFilter === 'up' ? 'bg-green-50 text-green-700' : 'text-gray-500 hover:text-green-600'}`}
                                    >
                                        Positive
                                    </button>
                                    <button 
                                        onClick={() => setFeedbackFilter('down')} 
                                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${feedbackFilter === 'down' ? 'bg-red-50 text-red-700' : 'text-gray-500 hover:text-red-600'}`}
                                    >
                                        Negative
                                    </button>
                                </div>
